...
class CreateHouseIntroducesTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('house_introduces', function (Blueprint $table) {
            ...
            $table->tinyInteger('floor_space')->default(1)->comment('占地面积');
            ...
            $table->timestamp('delivery_time')->default('1999-01-01 23:59:59')->comment('交付日期');

            $table->string('traffic', 250)->default('')->comment('交通配套介绍');
            $table->string('education', 250)->default('')->comment('教育配套介绍');
            $table->string('medical', 250)->default('')->comment('医疗配套介绍');
            $table->string('business', 250)->default('')->comment('商业配套介绍');
            $table->string('other' , 250)->default('')->comment('其他配套介绍');
            $table->timestamps();
        });
    }
