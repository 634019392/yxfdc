                    ...
                    <h2 style="margin: 5px 0px; ">
                        <font color="#333333" style="line-height: 20px; ">
                            <font style="line-height: 22px; " size="4">
                                亲爱的 {{ $user->truename }}</font>
                        </font>
                    </h2>
                    <p>首先感谢您加入本站！下面是您的账号信息<br>
                        您的账号：<b>{{ $user->username }}</b><br>
                        您的密码：<b>{{ $password }}</b><br>
                        您的邮箱：<b>{{ $user->email }}</b><br>
                        您的手机号：<b>{{ $user->phone }}</b><br>
                        您注册时的日期：<b>{{ $user->created_at }}</b><br>
                        当您在使用本网站时，遵守当地法律法规。<br>
                        如果您有什么疑问可以联系管理员，Email: {{ env('MAIL_USERNAME') }}</p>
                    {{--<p align="right">BER分接口网</p>--}}
                    <p align="right">{{ env('APP_NAME') }}接口网</p>
                    <p align="right">{{ date('Y年m月d日 H时i分s秒') }}</p>
