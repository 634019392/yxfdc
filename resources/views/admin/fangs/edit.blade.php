...
@section('content')
    ...
    <article class="page-container">
        ...
        <form action="{{ route('admin.fangs.update', $fang) }}" method="post" class="form form-horizontal" id="fang-add">
            @csrf
            {{ method_field('put') }}
            <div class="row cl">
                <label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>房源名称：</label>
                <div class="formControls col-xs-8 col-sm-9">
                    <input type="text" class="input-text" id="fang_name" name="fang_name" value="{{ $fang->fang_name }}">
                </div>
            </div>
            <div class="row cl">
            ...
            <div class="row cl">
                ...
                <div class="fang-style formControls col-xs-8 col-sm-9">
                    <select name="fang_province" style="width: 120px;" onchange="selectCity(this,'fang_city')">
                        ...
                        @foreach($cityData as $item)
                            <option @if($item->id == $fang->fang_province) selected @endif value="{{ $item->id }}">{{ $item->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="fang-style formControls col-xs-offset-4 col-sm-offset-3 col-xs-8 col-sm-9">
                    <select name="fang_city" id="fang_city" style="width: 100px;" onchange="selectCity(this,'fang_region')">
                        ...
                        @foreach($currentCityData as $item)
                            <option @if($item->id == $fang->fang_city) selected @endif value="{{ $item->id }}">{{ $item->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="fang-style formControls col-xs-offset-4 col-sm-offset-3 col-xs-8 col-sm-9">
                    <select name="fang_region" id="fang_region" style="width: 100px;">
                        ...
                        @foreach($currentRegionData as $item)
                            <option @if($item->id == $fang->fang_region) selected @endif value="{{ $item->id }}">{{ $item->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="fang-style formControls col-xs-offset-4 col-sm-offset-3 col-xs-8 col-sm-9">
            ...
            <div class="row cl">
                ...
                <div class="formControls col-xs-8 col-sm-9">
                    <input type="number" class="input-text" value="{{ $fang->fang_rent }}" style="width: 200px;" name="fang_rent"> 元
                </div>
            </div>
            <div class="row cl">
                <label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>楼层：</label>
                <div class="formControls col-xs-8 col-sm-9">
                    <input type="number" class="input-text" value="{{ $fang->fang_floor }}" style="width: 200px;" name="fang_floor">
                </div>
            </div>
            <div class="row cl">
            ...
            <div class="row cl">
                ...
                <div class="formControls col-xs-8 col-sm-9">
                    <input type="text" onfocus="WdatePicker({dateFmt:'yyyy'})" value="{{ $fang->fang_year }}" name="fang_year" class="input-text Wdate" style="width:120px;">
                </div>
            </div>
            <div class="row cl">
                ...
                <div class="formControls col-xs-8 col-sm-9">
                    @foreach($fang_config_data as $item)
                        <label>
                            <input @if(in_array($item->id, $fang->fang_config)) checked @endif type="checkbox" name="fang_config[]" value="{{ $item->id }}"/>
                            {{ $item->name }} &nbsp;&nbsp;
                        </label>
                    @endforeach
                </div>
            ...
            <div class="row cl">
                ...
                <div class="formControls col-xs-6 col-sm-7">
                    ...
                    <!-- 显示上传成功后的图片容器 -->
                    <div id="imglist">
                        @if(!empty($fang->images))
                            @foreach($fang->images as $item)
                                <span style="position: relative;margin-right: 30px">
                                    <img src="{{$item}}" style="width: 100px;height: 100px;"/>
                                    <strong onclick='delpic(this, "{{$item}}")' style="position: absolute;color: #1F1F1F;left: 110px;cursor:pointer;">X</strong>
                                </span>
                            @endforeach
                        @endif
                    </div>
